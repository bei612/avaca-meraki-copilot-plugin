/**
 * Temporal Workflow API - 设备状态工作流
 * 调用 DeviceStatusWorkflow 获取设备状态分布（饼图）
 */
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<DeviceStatusResponse>
) {
  try {
    // 限流检查
    ...
    // 解析请求参数
    const params: DeviceStatusRequest = req.body || {};
    const org_id = params.org_id || "850617379619606726"; // 默认 Concordia 组织ID

    console.log('[temporal-api] 调用设备状态工作流:', { org_id });
    ...
    // 启动工作流
    const handle = await client.workflow.start('DeviceStatusWorkflow', {
      args: [{ org_id }],
      taskQueue: 'meraki-workflows-queue',
      workflowId: `device-status-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
    });
