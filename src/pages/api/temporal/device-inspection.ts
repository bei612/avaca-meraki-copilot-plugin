/**
 * Temporal Workflow API - 设备巡检报告工作流
 * 调用 DeviceInspectionWorkflow 获取设备巡检报告（雷达图）
 */

import type { NextApiRequest, NextApiResponse } from 'next';
import { Connection, Client } from '@temporalio/client';

interface DeviceInspectionRequest {
  org_id?: string;
}

interface DeviceInspectionResponse {
  data?: any;
  error?: string;
  success: boolean;
  timestamp: number;
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<DeviceInspectionResponse>
) {
  try {
    // 限流检查
    ...
    // 解析请求参数
    const params: DeviceInspectionRequest = req.body || {};
    const org_id = params.org_id || "850617379619606726";

    console.log('[temporal-api] 调用设备巡检报告工作流:', { org_id });
    ...
    // 启动工作流
    const handle = await client.workflow.start('DeviceInspectionWorkflow', {
      args: [{ org_id }],
      taskQueue: 'meraki-workflows-queue',
      workflowId: `device-inspection-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
    });
    ...
    console.error('[temporal-api] 设备巡检报告工作流执行失败:', error);
