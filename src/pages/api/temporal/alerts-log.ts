/**
 * Temporal Workflow API - 告警日志工作流
 * 调用 AlertsLogWorkflow 获取告警日志（热力图）
 */

import type { NextApiRequest, NextApiResponse } from 'next';
import { Connection, Client } from '@temporalio/client';

interface AlertsLogRequest {
  org_id?: string;
}
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<AlertsLogResponse>
) {
  try {
    // 限流检查
    ...
    // 解析请求参数
    const params: AlertsLogRequest = req.body || {};
    const org_id = params.org_id || "850617379619606726";

    console.log('[temporal-api] 调用告警日志工作流:', { org_id });
    ...
    // 启动工作流
    const handle = await client.workflow.start('AlertsLogWorkflow', {
      args: [{ org_id }],
      taskQueue: 'meraki-workflows-queue',
      workflowId: `alerts-log-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
    });
    ...
    console.error('[temporal-api] 告警日志工作流执行失败:', error);
