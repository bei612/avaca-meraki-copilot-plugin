/**
 * Temporal Workflow API - 运维故障诊断工作流
 * 调用 TroubleshootingWorkflow 获取运维故障诊断分析（多图表组合）
 */
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<TroubleshootingResponse>
) {
  try {
    // 限流检查
    ...
    // 解析请求参数
    const params: TroubleshootingRequest = req.body || {};
    const org_id = params.org_id || "850617379619606726"; // 默认 Concordia 组织ID

    console.log('[temporal-api] 调用运维故障诊断工作流:', { org_id });
    ...
    // 启动工作流
    const handle = await client.workflow.start('TroubleshootingWorkflow', {
      args: [{ org_id }],
      taskQueue: 'meraki-workflows-queue',
      workflowId: `troubleshooting-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
    });
    ...
    console.error('[temporal-api] 运维故障诊断工作流执行失败:', error);
