/**
 * Temporal Workflow API - 许可证详情工作流
 * 调用 LicenseDetailsWorkflow 获取许可证状态（仪表盘）
 */

import type { NextApiRequest, NextApiResponse } from 'next';
import { Connection, Client } from '@temporalio/client';

interface LicenseDetailsRequest {
  org_id?: string;
}
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<LicenseDetailsResponse>
) {
  try {
    // 限流检查
    ...
    // 解析请求参数
    const params: LicenseDetailsRequest = req.body || {};
    const org_id = params.org_id || "850617379619606726";

    console.log('[temporal-api] 调用许可证详情工作流:', { org_id });
    ...
    // 启动工作流
    const handle = await client.workflow.start('LicenseDetailsWorkflow', {
      args: [{ org_id }],
      taskQueue: 'meraki-workflows-queue',
      workflowId: `license-details-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
    });
    ...
    console.error('[temporal-api] 许可证详情工作流执行失败:', error);
