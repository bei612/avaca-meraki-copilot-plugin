/**
 * Temporal Workflow API - 固件版本汇总工作流
 * 调用 FirmwareSummaryWorkflow 获取固件版本汇总（堆叠柱状图）
 */

import type { NextApiRequest, NextApiResponse } from 'next';
import { Connection, Client } from '@temporalio/client';

interface FirmwareSummaryRequest {
  org_id?: string;
}

interface FirmwareSummaryResponse {
  data?: any;
  error?: string;
  success: boolean;
  timestamp: number;
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<FirmwareSummaryResponse>
) {
  try {
    // 限流检查
    ...
    // 解析请求参数
    const params: FirmwareSummaryRequest = req.body || {};
    const org_id = params.org_id || "850617379619606726";

    console.log('[temporal-api] 调用固件版本汇总工作流:', { org_id });
    ...
    // 启动工作流
    const handle = await client.workflow.start('FirmwareSummaryWorkflow', {
      args: [{ org_id }],
      taskQueue: 'meraki-workflows-queue',
      workflowId: `firmware-summary-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
    });
    ...
    console.error('[temporal-api] 固件版本汇总工作流执行失败:', error);
